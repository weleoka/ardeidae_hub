...
HubControl.prototype = {
  ...
  setNewServer: function (data) {
    ...
    this.SysLog.file('Creating new server with id: ' + newServerId + ' From origin: ' + data.domain);
    ...
    for ( i = 0; i < this.hubArray.length; i++ ) {
      if ( this.hubArray[i].hasOwnProperty('updated') ) {
        ...
        if ( timeSinceRefresh > serverTTL ) {
          this.SysLog.file("Removing old server with id: " + i + " From origin: " + this.hubArray[i].domain);
